...
'use strict'
angular
.module('gpro.popupDetailProduitBCVente', [])
.controller('DetailProduitCommandeVenteController', ['$rootScope','$scope', '$modal', '$bootbox', '$log','$filter','baseUrl','$http','$routeParams','popCouleurTailleVenteService' ,
		...
		$scope.open = function (size,indexLigne,listProduitCommande,mode) {
			var modalInstance = $modal.open({
				...
				controller: function ($scope, $modalInstance, items) {
					...
					if(listProduitCommande[indexLigne].listDetailsProduitCommande != null){
					$log.debug( "CASE Detail : 1 =>> listDetail Pleinne : updateListeDetail");
						$rootScope.maGprogetMatrixEdit = listProduitCommande[indexLigne].listDetailsProduitCommande;
							...
							function(listTailleParProd){
								...
								if(angular.isDefined(listProduitCommande[indexLigne].id)){
									...
									if(listProduitCommande[indexLigne].id != ""){
										...
										tab = listProduitCommande[indexLigne].listDetailsProduitCommande;
				...
				$scope.ok = function () {
					...
					for ( var i=0 ;i<$rootScope.maGproDetailPrCmd.length ; i++){
						...
						listProduitCommande[indexLigne].listDetailsProduitCommande = tmpListMatrix; 
					};

					$log.debug('VALIDATION listDetail =>>  '+JSON.stringify(listProduitCommande[indexLigne].listDetailsProduitCommande,null,"  "));
