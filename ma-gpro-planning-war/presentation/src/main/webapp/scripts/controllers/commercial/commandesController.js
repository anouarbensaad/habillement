'use strict'
var app = angular.module('gpro.commandes', []);
app.controller(
				'commandesController',
				[
						'$scope',
								...
								$http
										...
										.success(
												function(datagetOF) {
													...
													$scope.ordreFabricationCourant.numeroInterne = datagetOF.numeroInterne;
													$scope.ordreFabricationCourant.prixUnitaire = datagetOF.prixUnitaire;
													$scope.ordreFabricationCourant.type = datagetOF.type;
													$scope.ordreFabricationCourant.produitReference = datagetOF.produitReference;
...
//							                                 $scope.myDataOF[i]  =  $scope.myDataOF[index] ? angular
//							         									.copy(ordreF)
//							         								 		: {};
							                                 break;
							                             }
						...
						//	$scope.listeOrdreFabrication();
							...
							var url = baseUrlGpao+"/report/getBesoinOF?ofId=" + idOF+"&type=pdf";
							...
							$scope.downloadAllOrdreFabrication = function(ordreFabricationCourant) {
								...
								////$log.debug("-- OFCourant " + JSON.stringify(ordreFabricationCourant, null, "  ") );
				       			url = baseUrlGpao + "/reportPlanning/listCommande?chaine=" + ordreFabricationCourant.vNumero
								 					 + "&clientId=" + ordreFabricationCourant.vCompositionBC
													 + "&produitId="+ordreFabricationCourant.vEtat
													 + "&dateLivraisonDu="+newdateLivOFMinFormat
													 + "&dateLivraisonTo="+newdateLivOFMaxFormat
													 + "&siteId="+ordreFabricationCourant.idSite
													 + "&vEtat="+ordreFabricationCourant.produitId													 
													 + "&type=pdf";
							...
							$scope.$watch(
								'myDataOF',
									function() {
										...
										{
											field : 'lieuLivraison',
											displayName : 'Lieu de livraison',
											width:'8%'
										},
//										{
//											field : 'informationsAffectation',
//											displayName : 'Info. Affectation',
//											width:'8%'
//										},
										{
											field : 'prixUnitaire',
